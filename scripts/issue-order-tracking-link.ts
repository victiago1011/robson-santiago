 ...
 * 4. Run:
 *      npx tsx --env-file=.env.local scripts/issue-order-tracking-link.ts --order-id <uuid>
...
function usage(): never {
  console.error(
    "Usage: npx tsx --env-file=.env.local scripts/issue-order-tracking-link.ts --order-id <uuid>",
  );
  process.exit(1);
}
...
async function main(): Promise<void> {
  ...
  const result = await issueOrderTrackingAccess(orderId, supabaseOrderTrackingStore);
  if (result.status !== "created") {
    console.error(`Could not issue tracking access: ${result.reason}`);
    process.exit(1);
  }

  const snapshot = await supabaseOrderTrackingStore.findOrderSnapshot(orderId);
